(function () {
  ...
  root.innerHTML = `
    <div id="pa-box" style="display:none;background:#fff;border:1px solid #ddd;border-radius:16px;box-shadow:0 12px 34px rgba(0,0,0,.16);overflow:hidden;">
      ...
      <div id="pa-log" style="height:340px;overflow:auto;padding:12px;display:flex;flex-direction:column;gap:10px;background:#fafbfc;"></div>
  ...
  function bubble(text, who = "bot") {
    ...
    // ✅ Phase 1: render line breaks + clickable links
    b.innerHTML = String(text)
      .replace(/&/g, "&amp;")
      .replace(/</g, "&lt;")
      .replace(/>/g, "&gt;")
      .replace(/\n/g, "<br>")
      .replace(/(https?:\/\/[^\s<]+)/g, '<a href="$1" target="_blank" rel="noopener noreferrer" style="color:inherit;text-decoration:underline;font-weight:700;">$1</a>');
